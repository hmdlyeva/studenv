...
const FilterSection = ({ theme }: Props) => {
  ...
  const handleTagRemove = (tag: Tag) => {
    setSelectedTags(
      selectedTags.filter((selectedTag: Tag) => selectedTag.title !== tag.title)
    );
  };
  return (
    <div
    ...
    >
      <div className="up container flex justify-between items-center">
        <div
          className={`input md:w-2/5 w-[95%] mx-auto border rounded-sm flex items-center p-2 gap-2 my-6 ${
            theme === "white"
              ? "bg-whitesecond"
        ...
        >
          <input
            ...
            className={`w-full ps-2 focus:outline-none ${
              theme === "white"
                ? "bg-whitesecond"
                : "bg-secondblack border-gray-600"
            }`}
        ...
        <p className="w-2/5 md:block hidden">
          Your search for <strong>&quot;Human Trafficking&quot;</strong> returns
          0 results
        </p>
      </div>
      <div
        ...
        <div className="right flex items-center gap-4 mx-4">
          <div className="tags lg:flex flex-row items-center gap-4 md:visible hidden">
            {selectedTags?.map((tag: { title: string }, i) => (
              <div
                key={i}
                className={`tag cursor-pointer gap-2 flex py-1 px-2 rounded-md items-center bg-blue-200 ${
                  theme === "white"
          ...
          <div
            className={`plus cursor-pointer rounded-md border p-1 flex justify-center items-center relative ${
              theme === "white" ? "bg-white" : "bg-dark border-gray-600"
            }`}
          ...
          >
            ...
            {addModal && (
              <div
                className={`tags flex flex-col w-60 absolute top-[42px] right-0 border rounded-xl px-2 h-52 overflow-y-auto scrollbar-none ${
                  theme === "white"
                    ? "bg-whitesecond"
                    : "bg-secondblack border-gray-600"
                }`}
